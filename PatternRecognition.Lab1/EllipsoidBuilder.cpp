...
{
	std::vector<Point2D> conv = convexHullBuilder->BuildConvHull(points);
...
Ellipsoid2D EllipsoidBuilder::BuildEllipsoid(const Rectangle2D & rect, std::vector<Point2D> points)
{
	...
	points = TranslatePoints(points, offset[0], offset[1]);
	...
	//static int i = 0;
	//std::cout<<(i%2 == 0 ? "Min: ": "Standart:") << alpha<<std::endl;
	...
	/*
	std::cout<<(i%2 == 0 ? "Min: ": "Standart:") << R<<std::endl;
	++i;*/
